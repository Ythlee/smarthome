...
char macAddr[32] = "01:11:22:33:44:55:66:77";//mac地址
// #define DEBUG
...
int DoAlarm(int sockfd, MsgStruct *msg, char *addr, int alarmType)
{
	...
	msg->frag_flag = MSG_FLAG_FRAG_NO;
	msg->frag_offset = 0;
	msg->custom1 = MSG_CUSTOM1_DEVICE_ALARM_NOTIFY_AIRCONDITION;
...
int SetDeviceStatus(int acceptfd, MsgStruct *msg)
{
	...
	//Get Result
	if(msg->custom1 != MSG_CUSTOM1_SET_DEVICE_AIRCONDITION )
	{
		printf("Device is not Aircondition!\n");
		wrong_code = 1;
	}
	char mac[32] = {0};
	...
	if(wrong_code == -1)
	{
		switch (msg->custom2)
		{
			case MSG_CUSTOM2_SET_DEVICE_AIR_ON :
				printf("Aircondition Set:ON.\n");
				//待补充
				break;
			case MSG_CUSTOM2_SET_DEVICE_AIR_OFF:
				printf("Aircondition Set:OFF.\n");
				break;
			case MSG_CUSTOM2_SET_DEVICE_AIR_HOT:
				printf("Aircondition Set:Hot Mode.\n");
				break;		
			case MSG_CUSTOM2_SET_DEVICE_AIR_COLD:
				printf("Aircondition Set:Cold Mode.\n");
				break;	
			case MSG_CUSTOM2_SET_DEVICE_AIR_TEMP:
				printf("Aircondition Temp Set:%s.\n", leftvalue);
				break;	
			case MSG_CUSTOM2_SET_DEVICE_AIR_WIND:
				printf("Aircondition Wind Set:%s.\n", leftvalue);
				break;	
			default:
...
static char* simulateOnOff[2] = {"on","off"};
static char* simulateHotCold[2] = {"hot", "cold"};
static char* simulateTemp[5] = {"25", "26", "27", "28", "29"};
static char* simulateWind[3] = {"low", "middle", "high"};
static int GetSimulateData(int msgType, char* msgData)
{
	switch (msgType)
	{
		case MSG_CUSTOM2_GET_DEVICE_AIR_ONOFF:
			strcpy(msgData, simulateOnOff[random()%2]);
			break;
		case MSG_CUSTOM2_GET_DEVICE_AIR_HOTCOLD:
			strcpy(msgData, simulateHotCold[random()%2]);
			break;
		case MSG_CUSTOM2_GET_DEVICE_AIR_TEMP:
			strcpy(msgData, simulateTemp[random()%5]);
			break;		
		case MSG_CUSTOM2_GET_DEVICE_AIR_WIND:
			strcpy(msgData, simulateWind[random()%3]);
			break;		
		default:
			printf("unknow setting:0x%x\n", msgType);
			break;
...
int GetDeviceStatus(int acceptfd, MsgStruct *msg)
{
	...
	//Get Result
	if(msg->custom1 != MSG_CUSTOM1_GET_DEVICE_AIRCONDITION)
	{
		printf("Device is not Aircondition!\n");
		wrong_code = 1;
	}
	...
	if(wrong_code == -1)
	{
		printf("Aircondition Device State Get\n");
		switch (get_order)
		{
			case MSG_CUSTOM2_GET_DEVICE_AIR_ONOFF:
				GetSimulateData(get_order, msg->data);
				printf("Aircondition ON or OFF:%s.\n", msg->data);
				break;
			case MSG_CUSTOM2_GET_DEVICE_AIR_HOTCOLD:
				GetSimulateData(get_order, msg->data);
				printf("Aircondition Hot or Cold Mode:%s.\n", msg->data);
				break;
			case MSG_CUSTOM2_GET_DEVICE_AIR_TEMP:
				GetSimulateData(get_order, msg->data);
				printf("Aircondition Temp:%s.\n", msg->data);
				break;		
			case MSG_CUSTOM2_GET_DEVICE_AIR_WIND:
				GetSimulateData(get_order, msg->data);
				printf("Aircondition Wind:%s.\n",msg->data);
				break;		
			default:
				printf("unknow setting:0x%x\n", get_order);
				wrong_code = 1;
...
static void* DealwithAlarmServer(void *arg)
{
	...
	while (1)
	{
		...
		if((i++)%2 == 0)
		{
			alarmType = MSG_CUSTOM2_DEVICE_ALARM_NOTIFY_AIR_CLOSE;
			printf("MSG_CUSTOM2_DEVICE_ALARM_NOTIFY_AIR_CLOSE.\n");
		}
		else
		{
			alarmType = MSG_CUSTOM2_DEVICE_ALARM_NOTIFY_AIR_TEMP;
			printf("MSG_CUSTOM2_DEVICE_ALARM_NOTIFY_AIR_TEMP.\n");
		}
